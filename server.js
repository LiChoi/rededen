...
const siteURL = 'https://red-eden.glitch.me'; //before, the client side URL was localhost:8080, and backend was localhost:3000. It'll all have the same URL now 
...
/******************************New stuff added from original files ported over from home computer*******************************************************************/

//This route will serve up the index.html file on the home route. Originally, I had to run the app on different ports: 1 for the webpack dev server, and 1 for the backend server
//Client side webpack dev server would make api calls to  the backend server. The '/' route on client-side would return the Home page. 
//However, the '/' route on back-end side would initialize some of the state variables by pulling data from the database
//I will attempt to change my app such that '/' route serves up the index.html file, and I will add the rest of the server routes in here
//The old '/' route on the server will be changed to '/initialize' 
...
app.get('/transformed', (req, res) => res.sendFile('/build/transformed.js',{root:"."})); 
...
/*********************************************new stuff added**************************************************************/
        //Okay, not a great solution to the problem of separating client-side routing from server-side routing, but it works.
        //Without these routes below, the browser would send a request to the server for these routes. If these routes don't exist
        //on this here server, then it's gonna throw a 'can't GET' error.
        //Not a big deal... unless user refreshes while on a route other than '/', or if user directly posts URL for any route other than '/'
        //So, I set up all the routes below to handle this event. Basically, server will just redirect to the '/', which will load up the javascript
        //onto the browser so that browser can handle all the client-side routing 
...
/**********************the above is an inelegant solution, but it works***********************/
        ...
        app.post('/login', passport.authenticate('local',{ failureRedirect: siteURL+'/' }), function(req, res){
            ...
            //originally redirected to forum, but since using react router, /forum is not a real route, so website returns 'cannot GET '/forum'' error. I am changing this to redirect to '/' as a temporary measure
            //This doesn't completely solve the problem, however. If user is on '/forum' react-route, then refreshes, this will cause an error. Don't know why it didn't cause a problem on my own computer
            res.redirect(siteURL+'/'); 
          });
